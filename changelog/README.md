# Changelog

This project uses tagged releases for meaningful guide milestones. Small source,
wording, and link-maintenance changes can ship between releases.

## v1.0.1 - 2026-08-17

Added the missing IaaS path to the startup foundation:

- Virtual Machines and Virtual Machine Scale Sets in the compute decision tree;
- a managed-platform-versus-VM decision record;
- a startup-sized IaaS operating baseline; and
- a source-backed virtual-machine reference architecture.

## v1.0.0 - 2026-08-17

Complete journey-first field guide release.

### Added

- five new technology decision records covering infrastructure as code, data,
  tenancy, commitments, and managed services;
- production paths for workload identity federation, observability, backup and
  recovery, readiness review, Microsoft Foundry, AI capacity, and credit
  graduation;
- product-market-fit and scale journeys covering FinOps, compliance, platform
  engineering, multiple regions, and Azure landing-zone graduation; and
- three source-backed reference architectures with accessible SVG diagrams.

### Changed

- every previously planned guide-map and roadmap item is now available;
- the website now routes directly to production, AI, scale, and architecture
  content; and
- volatile pricing, quota, regional, and Preview details remain delegated to
  current official sources rather than copied into static guidance.

### v1.0 verification

The v1.0 content was reviewed against current Microsoft Learn, Microsoft for
Startups, Azure Architecture Center, and GitHub OIDC documentation on August 17,
2026.

## v0.1.0 - 2026-08-17

Initial public field guide release.

### Included

- first-48-hours journey from application through support readiness;
- Microsoft for Startups program and portal navigator;
- company-backed identity and administrative continuity guidance;
- cost guardrails and graduation planning;
- startup-sized platform foundation;
- Azure Container Apps versus Azure Kubernetes Service decision record;
- curated official and community resource paths;
- four accessible, responsive SVG decision diagrams;
- contribution, editorial, governance, security, and conduct policies; and
- automated Markdown and external-link validation.

### Verification

Technical and program claims were reviewed against current official Microsoft
documentation on August 16-17, 2026. Time-sensitive program, pricing, quota,
region, support, and Preview information must still be checked at the linked
official source before making a production decision.

[Back to the main guide](https://github.com/Azure-Startup-Field-Guide/guide)
